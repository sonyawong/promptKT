...
def main(params, args):
    ...
    (
        ...
        data_config, model_name, batch_size, fold, win200, args, re_mapping
    )
    ...
    if model_name in ["stosakt"]:
        model = load_model(
            model_name, model_config, data_config, emb_type, save_dir, train_args
        )
    else:
        for remove_item in [
            "use_wandb",
    ...
    if model_name in ["gpt4kt"]:
        ...
        window_testauc, window_testacc = evaluate_testset(
            model,
            test_window_loader,
            model_name,
            save_test_window_path,
