...
def model_forward(model, data, attn_grads=None):
    ...
    if (
        ...
        model_name in ["gpt4kt", "unikt"] and model.module.emb_type.find("qid_pt") != -1
    ):
        dcur, dgaps = data
    else:
        dcur = data
...
def train_model(
    ...
    for i in range(pretrain_epoch + 1, num_epochs + 1):
        ...
        rank0_print(
            f"Epoch: {i}, validauc: {validauc:.4f}, validacc: {validacc:.4f}, best epoch: {best_epoch}, best auc: {max_auc:.4f}, train loss: {loss_mean}, valid loss: {valid_loss_mean}, emb_type: {model.module.emb_type}, model: {model.module.model_name}, save_dir: {ckpt_path}, duration: {end_time-start_time:.4f}s"
        )
        rank0_print(
