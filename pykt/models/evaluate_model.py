...
def save_cur_predict_result_emb(dres, q, r, d, t, qemb, qhemb, m, sm, p):
    ...
    for i in range(0, t.shape[0]):
        ...
        results.append(
            str([qs, rs, ds, ts, ps, prelabels, auc, acc, sq, sqh])
        )  # , cthr, cphr, sque, sqh]))
    return "\n".join(results)
...
def evaluate_testset(
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if (
                model_name in ["dkt_forget", "parkt", "mikt"]
                and model.emb_type.find("time") != -1
            ):
                dcur, dgaps = data
            elif model_name in ["bakt_time"]:
                dcur, dgaps = data
            elif (
                model_name in ["gpt4kt", "unikt"]
                and model.emb_type.find("qid_pt") != -1
            ):
                dcur, dgaps = data
            else:
                dcur = data
            ...
            if model.model_name in que_type_models and model.model_name not in [
                "lpkt",
                "gnn4kt",
                "gpt4kt",
                "unikt",
            ...
            if model_name in ["cdkt"]:
                ...
                if model.module.emb_type.find("grad") != -1:
                    save_grad_path = (
                        f"./save_attn/{dataset_name}_save_grad_fold_{fold}.npz"
                    )
                    save_attn_path = (
                        f"./save_attn/{dataset_name}_save_attnweight_fold_{fold}.npz"
                    )
                    y = model(
                        dcur,
                ...
                if model.emb_type.find("qid_pt") != -1:
                    y = model(dcur, dgaps=dgaps)
                else:
                    y = model(dcur)
                y = y[:, 1:]
                ...
                y = y[:, 1:]
            elif model_name in que_type_models and model_name not in [
                "lpkt",
                "gpt4kt",
                "gnn4kt",
                "unikt",
            ...
            if save_path != "":
                if model_name not in ["bakt_time"]:
                    ...
                    result = save_cur_predict_result_emb(
                        dres, c, r, cshft, rshft, qemb, qhemb, m, sm, y
                    )
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if (
                model_name in ["dkt_forget", "parkt", "mikt"]
                and model.module.emb_type.find("time") != -1
            ):
                dcur, dgaps = data
            elif model_name in ["bakt_time"]:
                dcur, dgaps = data
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn", "skvmn"]:
        ...
        p = torch.sigmoid(
            model.module.pred(model.module.dropout_layer(curhs[0]))
        ).squeeze(-1)
    elif model_name == "kqn":
        ...
        p = (
            model.module.sig(
                model.module.linear_5(torch.cat((curhs[1], curhs[0]), 1))
            ).sum(1)
            / model.module.d_k
        )
    return p
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    for ui in df:
        ...
        dcur["concept_preds"].append(
            ",".join([str(round(s, 4)) for s in (curdf["preds"].tolist())])
        )
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkvmn", "skvmn"]:
                ...
                y, h, dic_emb, num_c = model(
                    dcurori, dgaps, qtest=True, train=False, dic_emb=dic_emb
                )
                y = y[:, 1:]
                ...
                (
                    ...
                    mean_sequence_emb,
                    cov_sequence_emb,
                ) = model(dcurori, qtest=True, train=False)
            elif model_name in ["bakt"]:
                if model.module.emb_type.find("grad") != -1:
                    save_grad_path = (
                        f"./save_attn/{dataset_name}_save_grad_fold_{fold}.npz"
                ...
                start_hemb = (
                    torch.tensor([-1] * (ek.shape[0] * ek.shape[2]))
                    .reshape(ek.shape[0], 1, ek.shape[2])
                    .to(device)
                )
                ...
                cit = torch.cat(
                    (dcurori["itseqs"][:, 0:1], dcurori["shft_itseqs"]), dim=1
                )
                y, h, e_data = model(
                    cq.long(), cr.long(), cit.long(), at_data=None, qtest=True
                )
                start_hemb = (
                    torch.tensor([-1] * (h.shape[0] * h.shape[2]))
                    .reshape(h.shape[0], 1, h.shape[2])
                    .to(device)
                )  # add the first hidden emb
            ...
            for key in history_keys:
                if len(dhistory) == 0:
                    ...
                    if key == "hs":
                        ...
                        if model_name == "kqn":
                            dmerge[key] = [[], []]
                            dmerge[key][0] = torch.cat(
                                (dhistory[key][0], dcur[key][0]), dim=0
                            )
                            dmerge[key][1] = torch.cat(
                                (dhistory[key][1], dcur[key][1]), dim=0
                            )
                        elif model_name == "lpkt":
                            dmerge[key] = [[], []]
                            dmerge[key][0] = torch.cat(
                            ...
                            dmerge[key] = [
                                torch.cat((dhistory[key][0], dcur[key][0]), dim=0)
                            ]
                    else:
...
def MIKT_calC_INDEX(row, data_config):
    ...
    for idx, (s, t) in enumerate(zip(skills, timestamps)):
        ...
        curRepeatedGap = (
            data_config["num_rgap"] - 1
            if curRepeatedGap >= data_config["num_rgap"]
            else curRepeatedGap
        )
        repeated_gap.append(curRepeatedGap)
        cit_label.append(curCIt)
...
def get_info_dkt_forget(row, data_config, model_name, dataset_name):
    ...
    if model_name not in ["mikt"]:
        ...
        if dataset_name not in ["assist2009", "assist2015"]:
            rgap, sgap, pcount, its, tlabel, pretlabel, cit_label = MIKT_calC(
                row, data_config
            )
        else:
            rgap, sgap, pcount, its, tlabel, pretlabel, cit_label = MIKT_calC_INDEX(
                row, data_config
            )
        (
            dforget["rgaps"],
            ...
            dforget["pretlabel"],
            dforget["citlabel"],
        ) = (rgap, sgap, pcount, its, tlabel, pretlabel, cit_label)
    return dforget
...
def evaluate_splitpred_question(
    ...
    with torch.no_grad():
        ...
        for i, row in df.iterrows():
            ...
            dforget = (
                dict()
                if model_name not in ["dkt_forget", "bakt_time", "parkt", "mikt"]
                else get_info_dkt_forget(row, data_config, model_name, dataset_name)
            )
            ...
            if model_name == "lpkt":
                if times != []:
                    ...
                    it_times = np.maximum(
                        np.minimum(
                            (np.array(times) - np.array(shft_times)) // 60, 43200
                        ),
                        -1,
                    )
                else:
                ...
                dataset_name = data_config["dpath"].split("/")[-1]
                with open(
                    f"/root/autodl-nas/project/pykt_nips2022/data/{dataset_name}/skills_difficult_{model.module.difficult_levels}.csv",
                    "r",
                    encoding="UTF8",
                ) as f:
                    reader = csv.reader(f)
                ...
                with open(
                    f"/root/autodl-nas/project/pykt_nips2022/data/{dataset_name}/questions_difficult_{model.module.difficult_levels}.csv",
                    "r",
                    encoding="UTF8",
                ) as f:
                    reader = csv.reader(f)
