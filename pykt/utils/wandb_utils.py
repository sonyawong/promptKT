...
class WandbUtils:
    ...
    def _init_wandb(self):
        ...
        self.invert_sweep_dict = dict(
            zip(list(self.sweep_dict.values()), list(self.sweep_dict.keys()))
        )
    ...
    def get_sweep_dict(self):
        ...
        for name in list(sweep_dict.keys()):
            if len(sweep_dict[name]) > 1:
                print(
                    f"Error!! we can not process the same sweep name {name}, we will not return those sweeps:{sweep_dict[name]}"
                )
                del sweep_dict[name]
            else:
    ...
    def get_df(
        ...
        key_cache_path = os.path.join(
            self.cache_dir, f"{sweep_id}_model_config_keys.json"
        )
    ...
    def get_sweep_info(self, id, input_type="sweep_name"):
        ...
        result = self.project.client.execute(query, variable_values=variables)[
            "project"
        ]["sweep"]
    ...
    def get_stop_index(
        self, df, metric="validauc", metric_type="max", min_run_num=200, patience=50
    ):
        finish = False
        not_improve_num, i, first_best_index = -1, -1, -1
        df[f"{metric}_precsion3"] = df[metric].apply(
            lambda x: round(x, 3)
        )  # 忽略 1e-3 级别的提升
        for i in range(min_run_num, len(df)):
            best_value = df[:i][f"{metric}_precsion3"].max()  # get best value
            first_best_index = df[df[f"{metric}_precsion3"] == best_value][
                "run_index"
            ].min()
        ...
        stop_info = {
            "finish": finish,
            "not_improve_num": not_improve_num,
            "stop_index": i,
            "first_best_index": first_best_index,
        }
        return stop_info

    def check_sweep_early_stop(
        self,
        id,
        input_type="sweep_name",
        metric="validauc",
        ...
        report = {
            "stop_cmd": "",
            "id": sweep_id,
            "sweep_name": self.invert_sweep_dict[sweep_id],
        }
        if force_check_df:
            df = self.get_df(
                sweep_id, input_type="sweep_id", only_finish=True
            )  # get sweep result
            report["df"] = df
        ...
        if sweep_status in ["CANCELED", "FINISHED"]:
            ...
            num_run = self.get_sweep_run_num(
                sweep_id, input_type="sweep_id"
            )  # get sweep run num
            ...
            if num_run < min_run_num:
                ...
                stop_info = self.get_stop_index(
                    df,
                    metric=metric,
                    metric_type=metric_type,
                    min_run_num=min_run_num,
                    patience=patience,
                )
                if stop_info["finish"]:
        ...
        if self.print_details:
            print(
                f"    details: {id} state is {report['state']},num of runs is {report['num_run']}"
            )
            print("-" * 60 + "\n")
        return report
    ...
    def check_sweep_list(
        ...
        def check_help(
            ...
            min_run_num=min_run_num,
            patience=patience,
            force_check_df=force_check_df,
        ):
            try:
                check_result = self.check_sweep_early_stop(
                    sweep_name,
                    input_type=input_type,
                    metric=metric,
                    metric_type=metric_type,
                ...
                print(
                    f"Check fail for {sweep_name},detiils are {traceback.format_exc()}"
                )
    ...
    def check_sweep_by_pattern(
        ...
        for sweep_name in self.sweep_keys:
            if sweep_name.startswith(sweep_pattern) or sweep_pattern == "all":
                sweep_key_list.append(sweep_name)
        check_result_list = self.check_sweep_list(
            sweep_key_list,
            metric=metric,
            metric_type=metric_type,
            min_run_num=min_run_num,
    ...
    def get_all_fold_name(
        self, dataset_name, model_name, emb_type="qid", train_mode="ft"
    ):
        # sweep_key_list = [
        #     f"{dataset_name}_{model_name}_{emb_type}_{fold}" for fold in range(5)
        # ]
        sweep_key_list = [f"{dataset_name}_{model_name}_{emb_type}_{train_mode}"]
        # if sweep_key_list in self.sweep_keys:
        #     print(sweep_key_list)
        # else:
        #     print(f"nooo {sweep_key_list}")
        sweep_key_list = [
            x for x in sweep_key_list if x in self.sweep_keys
        ]  # filter error
        return sweep_key_list

    def check_sweep_by_model_dataset_name(
        self,
        dataset_name,
        model_name,
        emb_type="qid",
    ...
    def get_df_by_model_dataset_name(
        self,
        dataset_name,
        model_name,
        emb_type="qid",
        ...
        print(f"{dataset_name, model_name, emb_type}")
        sweep_key_list = self.get_all_fold_name(dataset_name, model_name, emb_type)
        print(f"sweep_key_list:{sweep_key_list}")
        df_list = self.get_multi_df(
            sweep_key_list,
            drop_duplicate=drop_duplicate,
            drop_na=drop_na,
            only_finish=only_finish,
    ...
    def get_best_run(
        ...
        best_path = os.path.join(
            save_dir, f"{dataset_name}_{model_name}_{emb_type}_best.csv"
        )
        ...
        if os.path.exists(best_path) and not force_reget:
            ...
            sweep_name_list, raw_df_list = self.get_df_by_model_dataset_name(
                dataset_name, model_name, emb_type=emb_type, n_jobs=n_jobs
            )  # get all all
            df_list = []
            # crop after best run
    ...
    def get_model_run_time(
        ...
        check_result_list = self.check_sweep_by_model_dataset_name(
            dataset_name,
            model_name,
            emb_type,
            metric=metric,
    ...
    # 修改wandb配置文件
    def generate_wandb(
        self, dataset_name, model_name, emb_type, fpath, ftarget, model_path
    ):
        # with open(fpath, "r") as fin, open(ftarget, "w") as fout:
        ...
        #     # data["parameters"]["dataset_name"]["values"][0].replace("xes", dataset_name)
        #     yaml.dump(data, fout)

        with open(fpath, "r") as fin, open(ftarget, "w") as fout:
            dname = "name: " + "_".join(
                [dataset_name, model_name, emb_type, "prediction"]
            )
            fout.write(dname + "\n")
            data = fin.read()
            data = data.replace("xes", dataset_name)
            data = data.replace("model_path", model_path[0])
            fout.write(data)
        ...
        with open(sweep_shell, "a+") as fallsh:
            if generate_all:
                ...
                for f in files:
                    ...
                    fallsh.write(
                        pre + fpath + " -p {}".format(self.project_name) + "\n"
                    )
            else:
    ...
    def extract_best_models(
        ...
        for i, row in df.iterrows():
            ...
            # target_path = (
            #     f"./best_model_path/{dataset_name}/{model_name}/{tmp_model_path}"
            # )
        ...
        ftarget = os.path.join(
            pred_dir,
            "{}_{}_{}_pred.yaml".format(dataset_name, model_name, emb_type),
        )
        if eval_test:
            self.generate_wandb(
                dataset_name,
    ...
    def extract_prediction_results(
        self, dataset_name, model_name, emb_type="qid", print_std=True
    ):
        ...
        """
        all_res = self.get_df(
            "_".join([dataset_name, model_name, emb_type, "prediction"]),
            input_type="sweep_name",
        )
        ...
        repeated_auc_mean, repeated_auc_std = np.mean(repeated_aucs), np.std(
            repeated_aucs, ddof=0
        )
        repeated_acc_mean, repeated_acc_std = np.mean(repeated_accs), np.std(
            repeated_accs, ddof=0
        )
        repeated_winauc_mean, repeated_winauc_std = np.mean(
            repeated_window_aucs
        ), np.std(repeated_window_aucs, ddof=0)
        repeated_winacc_mean, repeated_winacc_std = np.mean(
            repeated_window_accs
        ), np.std(repeated_window_accs, ddof=0)
        key = dataset_name + "_" + model_name
        if print_std:
            print(
                key + "_repeated:",
                "%.4f" % repeated_auc_mean
                + "±"
                + "%.4f" % repeated_auc_std
                + ","
                + "%.4f" % repeated_acc_mean
                + "±"
                + "%.4f" % repeated_acc_std
                + ","
                + "%.4f" % repeated_winauc_mean
                + "±"
                + "%.4f" % repeated_winauc_std
                + ","
                + "%.4f" % repeated_winacc_mean
                + "±"
                + "%.4f" % repeated_winacc_std,
            )
        else:
            print(
                key + "_repeated:",
        ...
        try:
            ...
            question_auc_mean, question_auc_std = np.mean(question_aucs), np.std(
                question_aucs, ddof=0
            )
            question_acc_mean, question_acc_std = np.mean(question_accs), np.std(
                question_accs, ddof=0
            )
            question_winauc_mean, question_winauc_std = np.mean(
                question_window_aucs
            ), np.std(question_window_aucs, ddof=0)
            question_winacc_mean, question_winacc_std = np.mean(
                question_window_accs
            ), np.std(question_window_accs, ddof=0)
            key = dataset_name + "_" + model_name
            if print_std:
                print(
                    key + "_concepts:",
                    "%.4f" % question_auc_mean
                    + "±"
                    + "%.4f" % question_auc_std
                    + ","
                    + "%.4f" % question_acc_mean
                    + "±"
                    + "%.4f" % question_acc_std
                    + ","
                    + "%.4f" % question_winauc_mean
                    + "±"
                    + "%.4f" % question_winauc_std
                    + ","
                    + "%.4f" % question_winacc_mean
                    + "±"
                    + "%.4f" % question_winacc_std,
                )
            else:
                print(
                    key + "_concepts:",
        ...
        try:
            ...
            early_auc_mean, early_auc_std = np.mean(early_aucs), np.std(
                early_aucs, ddof=0
            )
            early_acc_mean, early_acc_std = np.mean(early_accs), np.std(
                early_accs, ddof=0
            )
            early_winauc_mean, early_winauc_std = np.mean(early_window_aucs), np.std(
                early_window_aucs, ddof=0
            )
            early_winacc_mean, early_winacc_std = np.mean(early_window_accs), np.std(
                early_window_accs, ddof=0
            )
            key = dataset_name + "_" + model_name
            if print_std:
                print(
                    ...
                    + "%.4f" % early_acc_std
                    + ","
                    + "%.4f" % early_winauc_mean
                    + "±"
                    + "%.4f" % early_winauc_std
                    + ","
                    + "%.4f" % early_winacc_mean
                    + "±"
                    + "%.4f" % early_winacc_std,
                )
            else:
        ...
        try:
            ...
            latemean_auc_mean, latemean_auc_std = np.mean(late_mean_aucs), np.std(
                late_mean_aucs, ddof=0
            )
            latemean_acc_mean, latemean_acc_std = np.mean(late_mean_accs), np.std(
                late_mean_accs, ddof=0
            )
            latemean_winauc_mean, latemean_winauc_std = np.mean(
                late_mean_window_aucs
            ), np.std(late_mean_window_aucs, ddof=0)
            latemean_winacc_mean, latemean_winacc_std = np.mean(
                late_mean_window_accs
            ), np.std(late_mean_window_accs, ddof=0)
            key = dataset_name + "_" + model_name
            if print_std:
                print(
                    key + "_latemean:",
                    "%.4f" % latemean_auc_mean
                    + "±"
                    + "%.4f" % latemean_auc_std
                    + ","
                    + "%.4f" % latemean_acc_mean
                    + "±"
                    + "%.4f" % latemean_acc_std
                    + ","
                    + "%.4f" % latemean_winauc_mean
                    + "±"
                    + "%.4f" % latemean_winauc_std
                    + ","
                    + "%.4f" % latemean_winacc_mean
                    + "±"
                    + "%.4f" % latemean_winacc_std,
                )
            else:
                print(
                    key + "_latemean:",
            ...
            latevote_auc_mean, latevote_auc_std = np.mean(late_vote_aucs), np.std(
                late_vote_aucs, ddof=0
            )
            latevote_acc_mean, latevote_acc_std = np.mean(late_vote_accs), np.std(
                late_vote_accs, ddof=0
            )
            latevote_winauc_mean, latevote_winauc_std = np.mean(
                late_vote_window_aucs
            ), np.std(late_vote_window_aucs, ddof=0)
            latevote_winacc_mean, latevote_winacc_std = np.mean(
                late_vote_window_accs
            ), np.std(late_vote_window_accs, ddof=0)
            key = dataset_name + "_" + model_name
            if print_std:
                print(
                    key + "_latevote:",
                    "%.4f" % latevote_auc_mean
                    + "±"
                    + "%.4f" % latevote_auc_std
                    + ","
                    + "%.4f" % latevote_acc_mean
                    + "±"
                    + "%.4f" % latevote_acc_std
                    + ","
                    + "%.4f" % latevote_winauc_mean
                    + "±"
                    + "%.4f" % latevote_winauc_std
                    + ","
                    + "%.4f" % latevote_winacc_mean
                    + "±"
                    + "%.4f" % latevote_winacc_std,
                )
            else:
                print(
                    key + "_latevote:",
            ...
            lateall_auc_mean, lateall_auc_std = np.mean(late_all_aucs), np.std(
                late_all_aucs, ddof=0
            )
            lateall_acc_mean, lateall_acc_std = np.mean(late_all_accs), np.std(
                late_all_accs, ddof=0
            )
            lateall_winauc_mean, lateall_winauc_std = np.mean(
                late_all_window_aucs
            ), np.std(late_all_window_aucs, ddof=0)
            lateall_winacc_mean, lateall_winacc_std = np.mean(
                late_all_window_accs
            ), np.std(late_all_window_accs, ddof=0)
            key = dataset_name + "_" + model_name
            if print_std:
                print(
                    key + "_lateall:",
                    ...
                    + "%.4f" % lateall_acc_std
                    + ","
                    + "%.4f" % lateall_winauc_mean
                    + "±"
                    + "%.4f" % lateall_winauc_std
                    + ","
                    + "%.4f" % lateall_winacc_mean
                    + "±"
                    + "%.4f" % lateall_winacc_std,
                )
            else:
                print(
                    key + "_lateall:",
            ...
            print(f"{model_name} don't have late fusion!!!")
